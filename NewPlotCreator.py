...
def plotClusteredMatrix(
    ...
    blockBlockMatrix = NewBlockBlockMatrix(leftClusters, rightClusters, A)
    ...
    if numPlots == 1:
        ...
        if plotInRows:
            ...
            fig = plt.figure(figsize=(12 * numPlots, 12))
    ...
    if plotUnorderedMatrix:
        idx = 0
        axs[idx].matshow(blockBlockMatrix.originalMatrix)

    if plotOrderedMatrix:
        idx = int(plotUnorderedMatrix) + int(plotOrderedMatrix) - 1
        blockBlockMatrix.plotOriginalMatrix(axs=axs[idx])

    if plotClusterMatrix:
        idx = numPlots - 1
        blockBlockMatrix.plotMatrix(axs=axs[idx])
